...
def get_restaurants():
    try:
        ...
        cursor.close()
        print("Successfully fetched restaurants:", restaurants)
        return restaurants
    except mysql.connector.Error as err:
        print("Error fetching restaurants:", err)
        return []

@app.route('/fetchrestaurants', methods=['GET'])
...
def adminPage():
    if 'user_id' in session:
        ...
        restaurants = get_restaurants()
        return render_template('adminPage.html', username=username, restaurants=restaurants)
